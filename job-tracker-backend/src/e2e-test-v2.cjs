...
const LOGFILE = path.join(__dirname, 'e2e-test-v2.log');
...
function runFullFlow(port) {
  ...
  const steps = [
    { name: '1. Signup', method: 'POST', path: '/api/auth/signup', body: { name: 'E2EV2', email: 'e2ev2@tracker.test', password: 'test1234' } },
    { name: '2. Login', method: 'POST', path: '/api/auth/login', body: { email: 'e2ev2@tracker.test', password: 'test1234' } },
    ...
    const req = http.request({ hostname: 'localhost', port, path: targetPath, method: step.method, headers }, (res) => {
      ...
      res.on('end', () => {
        ...
        if (res.statusCode >= 400) {
          console.log('  FAIL: ' + data.substring(0, 250));
        } else {
          const p = data.length > 200 ? data.substring(0, 200) + '...' : data;
          console.log('  OK: ' + p.replace(/\n/g, ' '));
        }
        setTimeout(next, 200);
      });
    });
